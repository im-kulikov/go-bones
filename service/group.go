...
type (
	...
	// Group collects services and runs them concurrently.
...
// New creates and configures Group by passed Option's.
func New(log logger.Logger, options ...Option) Group {
	runner := &group{
		logger: log,
	...
	if runner.shutdown <= 0 {
		runner.shutdown = defaultShutdownTimeout
	}
...
func (g *group) stopServices(ctx context.Context, cause error, output <-chan stopper) {
